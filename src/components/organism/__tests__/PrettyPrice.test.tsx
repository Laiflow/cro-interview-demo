...
vi.mock("../PrettyAmountCoin", () => ({
  default: ({ amount, code, precision }) => (
    <span data-testid="pretty-amount">
      {`${amount} ${code} (precision: ${precision})`}
    </span>
  ),
}));
...
describe("PrettyPrice", () => {
  ...
  it("should render with amount prop", () => {
    ...
    // 2 BTC * 10000 = 20000
    expect(getByTestId("pretty-amount").textContent).toBe(
      "20000 BTC (precision: 2)",
    );
  });
  ...
  it("should use custom precision when provided", () => {
    const { getByTestId } = render(
      <PrettyPrice currency="BTC" amount={2} precision={4} />,
    );

    expect(getByTestId("pretty-amount").textContent).toBe(
      "20000 BTC (precision: 4)",
    );
  });
  ...
  it("should handle non-numeric children", () => {
    const { getByTestId } = render(
      <PrettyPrice currency="BTC">abc</PrettyPrice>,
    );
