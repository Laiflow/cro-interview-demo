...
describe('useSocketWithWorker', () => {
  ...
  it('should emit messages', async () => {
    const { result } = renderHook(() => useSocketWithWorker('wss://example.com/socket'))
  ...
  it('should handle manual option (not auto connect)', async () => {
    renderHook(() => useSocketWithWorker('wss://example.com/socket', { manual: true }))
    // manual为true时不会自动连接
  ...
  it('should handle connect and disconnect manually', async () => {
    ...
    expect(mockWorkerMessages).toHaveBeenCalledWith(expect.objectContaining({ action: 'CONNECT' }))
  ...
  it('should handle worker runtime errors', async () => {
    ...
    const { result } = renderHook(() => useSocketWithWorker('wss://runtime-error.example.com'))
